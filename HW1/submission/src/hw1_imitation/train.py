...
def train_one_step(state,action_chunk,model,optimizer):
...
def run_training(config: TrainConfig) -> None:
    ...
    print("Loaded data")
    # print("States shape: ", states.shape) #(25650, 5) -->   (x,y,angle,target_x,target_y)
    # print("Actions shape: ", actions.shape) #(25650, 2) --> (delta_x, delta_y)
    # print("Episode ends shape: ", episode_ends.shape)  #(206,) --> 206 different try to push T into target space
    # print("States look like \n", states[:5])
    # print("Actions look like \n", actions[:5])
    # print("Episode ends look like \n", episode_ends[:5])
    # print("what's the value in the episode ends? \n", np.unique(episode_ends))
    ...
    train_step = torch.compile(train_one_step)
    for epoch in range(config.num_epochs):
        for batch in loader:
            ...
            loss = train_step(state,action_chunk,model,optimizer)
            
            if step % config.log_interval == 0:
                logger.log({"train_loss": loss.item()}, step)
